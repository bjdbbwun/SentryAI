...
const AppLogo = ({ className = "h-10" }: { className?: string }) => (
  <img src="/image_0.png" alt="Obitrex Logo" className={className} referrerPolicy="no-referrer" />
);
...
function Dashboard({ user, profile, language, setLanguage, theme, setTheme }: any) {
  ...
  useEffect(() => {
    const savedHistory = localStorage.getItem('obitrex_history');
    if (savedHistory) {
      try {
  ...
  useEffect(() => {
    localStorage.setItem('obitrex_history', JSON.stringify(history));
  }, [history]);

  useEffect(() => {
    if (result) {
      localStorage.setItem('obitrex_last_result', JSON.stringify(result));
    } else {
      localStorage.removeItem('obitrex_last_result');
    }
  }, [result]);
  ...
  const handleScan = async () => {
    ...
    console.log("Obitrex: Initiating scan protocol for content length:", inputText.length);
    setIsScanning(true);
    setResult(null);
  ...
  const clearHistory = () => {
    setHistory([]);
    localStorage.removeItem('obitrex_history');
  };
